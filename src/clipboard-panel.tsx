    ...
    <>
      ...
      <div className="wrap">
        <div className="search">
          <input ref={searchRef} type="text" placeholder="搜索剪贴板历史…" spellCheck={false} value={keyword} onChange={(e) => { setKeyword(e.target.value); setSelected(0); }} />
        </div>
        <div className="tabs">
          {CATS.map((c) => (
            <button key={c.key} className={`tab ${category === c.key ? "on" : ""}`} onClick={() => { setCategory(c.key); setSelected(0); }}>
              {c.label}
            </button>
          ))}
        </div>
        ...
        <div className="hint">Enter/双击 粘贴 · 1-9 快选 · ↑↓ 移动 · 右键菜单 · Esc 关闭</div>
      </div>
...
function Row({ it, index, sel, thumb, icon, onClick, onDouble, onMenu }: { it: ClipItem; index: number | string; sel: boolean; thumb?: string; icon?: string; onClick: () => void; onDouble: () => void; onMenu: (x: number, y: number) => void }) {
  ...
  const sub = it.type === "image" ? `${it.preview} · ${(it.size / 1024).toFixed(0)}KB` : it.type === "files" ? `${it.size} 个文件` : it.sourceApp || `${it.size} 字符`;
  return (
...
function Preview({ item, appIcon }: { item?: ClipItem; appIcon?: string }) {
  ...
  if (!item) return <div className="empty">选择一个条目查看</div>;
  const meta = (
    <div className="pv-meta">
      {appIcon ? <img src={appIcon} alt="" /> : null}
      <span>{item.sourceApp || "未知来源"}</span>
      <span>·</span>
      <span>{item.type === "text" ? item.size + " 字符" : item.type === "image" ? item.preview : item.size + " 个文件"}</span>
      <span>·</span>
    ...
    return (
      <>
        <div className="pv-body">
          ...
          <div className="kv"><b>{fileName(item)}</b></div>
          <div className="kv">尺寸：{item.preview}</div>
          <div className="kv">大小：{(item.size / 1024).toFixed(1)} KB</div>
          {item.sourcePath ? <div className="kv">位置：{item.sourcePath}</div> : null}
        </div>
        {meta}
      </>
    );
...
function Menu({ menu, onClose }: { menu: { x: number; y: number; item: ClipItem }; onClose: () => void }) {
  ...
  return (
    <div className="menu" style={{ left: Math.min(menu.x, window.innerWidth - 150), top: Math.min(menu.y, window.innerHeight - 120) }} onClick={(e) => e.stopPropagation()}>
      <button onClick={() => act("copy")}>复制</button>
      <button onClick={() => act("fav")}>{it.favorite ? "取消收藏" : "收藏"}</button>
      <button className="del" onClick={() => act("del")}>删除</button>
    </div>
  );
}
