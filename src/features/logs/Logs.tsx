  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      <div className="flex items-center justify-between px-[22px] py-[14px] border-b border-border shrink-0 gap-3">
        <h1 className="m-0 text-[16px] font-semibold">{t("logs.title")}</h1>
        <button onClick={() => desktop.openLogsFolder()} className="flex items-center gap-1.5 px-3 py-1.5 border border-border bg-card text-text rounded-lg text-[12.5px] cursor-pointer">{t("logs.openFolder")}</button>
      </div>
      <div className="flex-1 overflow-y-auto px-[22px] py-[14px] font-mono text-[12px] leading-[1.95] min-h-0">
        {lines.length ? (
          ...
          <div className="text-muted">{t("logs.empty")}</div>
        )}
      </div>
    </div>
  );
