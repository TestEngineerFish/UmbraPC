...
//
// 分类图标的契约（批次 004 起）：服务端存**语义名**（gift / pet / …，用户新增分类时
// 挑的那一个），两端各自把语义名映射到自己的路径资源 —— 渲染时存储的语义名优先，
// 没存（内置分类、老数据）按 slug 兜底，兜不到用「其他」的三个点。
...
// 新增分类弹窗的首批八个可选图标（批次 004 稿）。k 是落库的语义名 ——
// 只有 k 进契约；path 是 PC 自己的资源，iOS 有同名的一份自己的 path。
export const PICK_ICONS: { k: string; label: string; d: string }[] = [
  { k: "gift", label: "礼物", d: "M20 12v9H4v-9M2.5 7.5h19v4.5h-19zM12 21.5V7.5M12 7.5H7.8a2.5 2.5 0 0 1 0-5c3.2 0 4.2 5 4.2 5M12 7.5h4.2a2.5 2.5 0 0 0 0-5c-3.2 0-4.2 5-4.2 5" },
  { k: "pet", label: "宠物", d: "M4.6 13.4a1.9 1.9 0 1 0 0-.1M9.2 8.4a1.9 1.9 0 1 0 0-.1M14.8 8.4a1.9 1.9 0 1 0 0-.1M19.4 13.4a1.9 1.9 0 1 0 0-.1M12 21c-3 0-5-1.8-5-4 0-2.1 1.9-4 5-4s5 1.9 5 4c0 2.2-2 4-5 4" },
  { k: "health", label: "健康", d: "M12 20.5S3.6 15 3.6 9.3A4.7 4.7 0 0 1 12 6.5a4.7 4.7 0 0 1 8.4 2.8c0 5.7-8.4 11.2-8.4 11.2" },
  { k: "sport", label: "运动", d: "M6.5 6.5v11M3.5 9v6M17.5 6.5v11M20.5 9v6M6.5 12h11" },
  { k: "coffee", label: "咖啡", d: "M17 8.5h1a3.8 3.8 0 1 1 0 7.6h-1M3.5 8.5H17v8.6a3.8 3.8 0 0 1-3.8 3.8H7.3a3.8 3.8 0 0 1-3.8-3.8zM6.5 2.5v2.4M10.2 2.5v2.4M13.9 2.5v2.4" },
  { k: "book", label: "书", d: "M4 19.5A2.5 2.5 0 0 1 6.5 17H20M6.5 2.5H20v19H6.5A2.5 2.5 0 0 1 4 19v-14a2.5 2.5 0 0 1 2.5-2.5" },
  { k: "tag", label: "标签", d: "M20.6 13.4 13.4 20.6a2 2 0 0 1-2.8 0l-7.2-7.2a2 2 0 0 1-.6-1.4V4.5a1 1 0 0 1 1-1H11a2 2 0 0 1 1.4.6l8.2 8.2a1.6 1.6 0 0 1 0 1.1M7.5 7.5h.01" },
  { k: "house", label: "房子", d: "M3 10.5 12 3.2l9 7.3M5.3 9.4V21h13.4V9.4M10 21v-6h4v6" },
];
...
export const catIcon = (slug: string, icon?: string): string =>
  (icon && PICK_BY_NAME[icon]) || CAT_ICON[slug] || CAT_ICON.other;
...
// （已退场）SUBS —— 硬编码的二级分类预设表。第二批「子类落库」后服务端是
// 唯一正本（money_subs 表，随 GET /money/categories 的 subs 字段下发），
// 本地表删除：留着它，用户在 iOS 加的子类这里永远看不见，两份来源迟早对不上。
// 二级仍是**中文字符串不是 slug**，流水里出现表外的二级依旧完全合法。
